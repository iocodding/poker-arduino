<template>
  <div>
    ...
    <div
      id="tiptap"
      class="divide-y divide-slate-200 hover:divide-slate-300 border rounded-md border-slate-200 hover:border-slate-300 focus:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600 dark:focus:border-slate-600 shadow-sm"
    >
      <div id="tiptap-toolbar" class="divide-x divide-slate-200">
        <TiptapToolbarGroup>
          <TiptapToolbarButton
            label="Heading 1"
...
const props = defineProps(["label", "content"]);
...
onMounted(() => {
  setTimeout(
    () => emit("update:content", editorInstance.value?.getHTML()),
    250
  );
});
